...
export default function Media() {
  ...
  const { isLoading: filtersLoading, data: filtersData } = useFilters("media");
  ...
  const {
    ...
    error: mediaError,
    isError,
  } = useItems("media", currentPage, appliedFilters);
            ...
            <div className="flex justify-center items-center min-h-[400px]">
              <div className="text-center p-4">
                ...
                <p className="text-gray-600">
                  {mediaError?.message || "Failed to load games"}
                </p>
              </div>
            </div>
          ) : (
            <Items data={data?.data || []} loading={dataLoading} type="media" />
          )}
        </main>
      </div>
